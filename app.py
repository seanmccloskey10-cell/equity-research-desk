"""Streamlit entry point — Equity Research Desk.
...
from lib.cache import cache_quote
...
@cache_quote
def _sidebar_quote(_orch: DataOrchestrator, ticker: str):
    """Cached quote for the sidebar watchlist (mirrors the views' cached quotes
    so the sidebar doesn't re-hit the network on every rerun)."""
    return _orch.get_quote(ticker)


def _sidebar(settings: Settings, orch: DataOrchestrator) -> None:
    st.sidebar.title("Equity Research Desk")
    ...
    if market.state != "regular":
        # Outside regular hours, yfinance serves the last close — say so plainly
        # so a non-live price never reads as broken or as a fresh move.
        st.sidebar.caption("Prices shown are the last close, not live.")

    st.sidebar.subheader("Watchlist")
    _symbols = [t.symbol for t in settings.tickers]
    _quotes = fetch_all(_symbols, lambda s: _sidebar_quote(orch, s))
    for t in settings.tickers:
        q = _quotes.get(t.symbol) or {}
        price = q.get("price")
