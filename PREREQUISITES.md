...
```
I'm about to install a small equity (stock) research tool and I want
to set up permissions + prerequisites FIRST so the actual install runs
with minimum interruption. I'm on macOS. I'm AI-fluent but not a
...
1. ENTER PLAN MODE FIRST. Run version checks and report what's
   ...
   installed and which is missing. Don't install anything yet.
...
2. PROPOSE an install plan that PREFERS GUI dialogs over terminal
   ...
   - Python 3.11 — prefer the official .pkg installer from python.org
     (uses Apple's GUI password dialog with lock icon) over
...
6. STOP. Confirm: git OK, python3 OK, code (VS Code) OK, plan auth OK.
   I'll paste the next prompt once you say "prerequisites complete".

(Optional — only if I ask: install the Claude Code CLI via
`npm install -g @anthropic-ai/claude-code` after Node.js is installed
via `brew install node`. But the install prompts work fine through
whichever Claude agent I'm already using — including the VS Code
extension chat panel — so don't push this on me.)
...
I'm about to install a small equity (stock) research tool and I want
to set up permissions + prerequisites FIRST so the actual install runs
with minimum interruption. I'm on Windows 11. I'm AI-fluent but not a
developer — you handle setup, I approve major actions.
...
Important Windows quirk for verification:
- Don't use bare `code --version` to check VS Code — on Windows in
  ...
  `code.cmd --version` from PowerShell, or `where.exe code` to see
  all candidates and pick the right one.
...
1. ENTER PLAN MODE FIRST. Run the version checks below and report
   ...
   - `where.exe code` (NOT `code --version` — see quirk above)
...
4. AFTER my "go", run the batched install. Tell me when each line
   completes. If UAC fires more than once, something's wrong with the
   batching — pause and tell me.
...
6. STOP. Confirm: git OK, python OK, code.cmd OK, plan auth OK. I'll
   paste the next prompt (the dashboard handoff) once you say
...
But the install prompts work fine through whichever Claude agent I'm
already using — including the VS Code extension chat panel — so don't
push this on me.)

Common Windows dialogs you'll see (pre-warning so I don't panic):
...
- SmartScreen on signed installers ("Windows protected your PC")
  -> "More info" -> "Run anyway" if it's from python.org or
  code.visualstudio.com (winget signed sources usually skip this)
- "Allow Python through Windows Defender Firewall?" -> Allow
...
Before pasting the handoff prompt, your agent should have confirmed:
...
Skip this file. Go straight to [HANDOFF-PROMPT.md](HANDOFF-PROMPT.md) — the handoff prompt itself checks prerequisites at the start, and if anything's missing it'll tell you to come back here.
