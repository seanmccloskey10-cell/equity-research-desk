...
- **Is port 8501 reachable?** Open http://localhost:8501 in the user's browser (macOS: `open http://localhost:8501`). If "site can't be reached", the dashboard isn't running.
...
- **Corporate network / VPN**: some networks block the Yahoo endpoint. If the user is on a corporate VPN, try toggling it off and retry.
...
- **Sidebar**: Title "Equity Research Desk", NYSE status pill with a colored dot (green = open, amber = pre/after, red = closed), one watchlist card per ticker (the template ships with a starter list: NVDA, AAPL, MSFT, GOOGL, TSLA) showing ticker + price + colored % change, "Data sources" section with a green dot next to Yahoo Finance.
- **Overview tab**: Markets row (S&P 500 + Nasdaq-100 benchmarks), watchlist table with one row per ticker — each row has a real price, a 30-day sparkline trend, a 52W position bar, and a Day% that's not `—`.
