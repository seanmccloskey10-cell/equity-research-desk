...
1. **Market context strip** — SPY + QQQ benchmarks (day %, YTD %). Shows
   the user whether the watchlist is moving with the market or independently.
...
def _ytd_pct(_orch: DataOrchestrator, ticker: str) -> Optional[float]:
    ...
    # Guard against a near-zero opening price (penny/degenerate data) producing
    # an absurd percentage — treat it as no data rather than render "+50000%".
    if first is None or pd.isna(first) or pd.isna(last) or abs(first) < 0.01:
        return None
...
def render(settings: Settings, orch: DataOrchestrator) -> None:
    ...
    # ---- Markets section ----
    # Market context first — so the user reads "market is up 0.8%" before they read
    # any individual ticker and can calibrate their reaction.
    ...
    _market = current_status()
    if _market.state == "closed":
        st.caption("📅 US market is closed — prices shown are the last close, not live.")
    elif _market.state in ("pre", "after"):
        st.caption("📅 Outside regular trading hours — prices may be the last close, not live.")
