...
def render(settings: Settings, orch: DataOrchestrator) -> None:
    ...
    with st.expander("📖 New to these terms? Plain-English glossary"):
        st.markdown(
            "- **P/E (price-to-earnings)** — share price ÷ earnings per share. Roughly how many years of profit you're paying for; lower can mean cheaper.\n"
            "- **Forward P/E** — the same, but using *expected* next-year earnings.\n"
            "- **P/B (price-to-book)** — price vs the company's net asset value. Lower = cheaper relative to assets.\n"
            "- **EPS (earnings per share)** — profit divided by the number of shares.\n"
            "- **Dividend yield** — annual dividend as a % of the share price (the income it pays you).\n"
            "- **ROE (return on equity)** — profit as a % of shareholders' money. Higher = more efficient.\n"
            "- **Profit margin** — profit as a % of revenue. Higher = keeps more of each dollar of sales.\n"
            "- **Debt/Equity** — borrowing vs shareholders' money. Higher = more leverage and risk.\n"
            "- **Beta** — how much the stock swings vs the market. Above 1 = more volatile, below 1 = calmer.\n"
            "- **Market cap** — total value of all shares (price × shares outstanding).\n"
            "- **52W position** — where today's price sits between the 52-week low (0%) and high (100%).\n"
            "- **RSI (0–100)** — momentum gauge. Above ~70 = 'overbought', below ~30 = 'oversold'.\n"
            "- **MACD** — trend gauge built from two moving averages; a 'bullish/bearish crossover' hints at a shift.\n"
            "- **SMA 20 / 50 / 200** — average price over the last 20 / 50 / 200 days; price above the 200-day line often signals an uptrend.\n"
        )
        st.caption("Educational summaries, not financial advice.")

    if not settings.tickers:
        st.info(
            "No tickers configured. Say to your agent: *\"Add AAPL to my watchlist.\"*"
        )
        return
