...
Common problems and how to fix them. Written for the user's Claude Code agent to scan when the user describes a problem — paste the symptom into the search box here, the agent executes the fix.

The tool assumes **macOS**, so macOS commands are shown first. Windows equivalents are in parentheses where relevant (for Windows users).
...
- **macOS**: TextEdit's default rich-text format. The agent should edit `.env` via its file tools, not TextEdit. If there's already a `.env.rtf`, delete it; the agent will re-copy `.env.example` → `.env` on next `python3 run.py setup`.
...
The Overview tab shows an explicit error banner when all tickers fail. If you see that banner, no action is needed other than waiting or checking network connectivity. If the user is on a corporate VPN, try disconnecting and retry.
...
### How do I reset the watchlist to the starter list?

This template ships with a starter watchlist of five well-known stocks. To get back to it, set `config/tickers.yaml` to:

```yaml
tickers:
  - symbol: NVDA
    name: NVIDIA
    note: Semiconductors — AI / accelerated computing
  - symbol: AAPL
    name: Apple
    note: Consumer hardware + services
  - symbol: MSFT
    name: Microsoft
    note: Software / cloud (Azure)
  - symbol: GOOGL
    name: Alphabet
    note: Search / ads / cloud
  - symbol: TSLA
    name: Tesla
    note: EVs / energy / autonomy
```

(Or just ask your agent: "reset my watchlist to the starter stocks.")
