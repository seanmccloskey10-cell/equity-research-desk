...
**You are Claude Code running in a user's cloned copy of this repo.** This file teaches you how the codebase is organized, what the non-negotiable rules are, and how to extend the tool safely. Read [PRD.md](PRD.md) for full product context, then come back here.
...
A local-first equity research dashboard. Python + Streamlit + Plotly. Runs on the user's laptop, displays live data for their watchlist (default: a starter list — NVDA, AAPL, MSFT, GOOGL, TSLA — the user edits in `config/tickers.yaml`). yfinance is the primary data source — **the tool works with zero API keys.**

---

## Who the user is

**A non-technical user.** They operate this tool entirely via natural-language prompts to you. They do not type commands, edit `.env` by hand, or debug stack traces. Your job is to be the translation layer between their intent and the code.

Assume **macOS** unless the user tells you otherwise — see the macOS gotchas section below. Windows notes are further down for Windows users.

Default to a **crypto-free** watchlist and examples unless the user explicitly asks for crypto — it's a safe, neutral default.
...
- Removing a data source the user already configured
- Deleting or renaming a view the user has been using
- Any change that would require the user to reconfigure keys they've already set
...
- Do not add crypto tickers or demo examples unless the user explicitly asks (crypto-free is the default)
...
See [APIS.md](APIS.md) for the full directory of financial APIs the user can evaluate — organized by problem (market data, news, fundamentals, macro, earnings, filings, insider/institutional, options, alternative). Includes recommendations, free-tier limits, paid tier pricing, and a decision framework. Sean's top-3 picks:

1. **Financial Modeling Prep** — deep fundamentals. Free 250/day, $19/mo unlimited.
2. **FRED** (St. Louis Fed) — unlimited free macro data.
3. **Quartr** — earnings call transcripts; also a Claude Cowork connector.

The non-API honest flag: **Koyfin** ($35/mo web dashboard) is the zero-build alternative if the user would rather not iterate on this tool. Don't recommend switching by default — tell the user it exists if they ask about visual-quality benchmarks.

When the user asks to add a new API, follow the "Add a new data source" pattern above and the walkthrough at the bottom of APIS.md.
...
- **Phase 10** — Full visual redesign + KaTeX bug fix + AI Brief per-section renderer + rename: ✅ shipped (2026-04-22)
  - **Rename**: tool is "Equity Research Desk" — page title ([app.py](app.py) `st.set_page_config`), sidebar title, and module docstring
