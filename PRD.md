# Equity Research Desk — PRD

**Owner:** Sean McCloskey
**For:** a non-technical user who operates the tool entirely through an AI coding agent
**Status:** Generic template — starter-watchlist (NVDA + 4 mega-caps) base case, ready to fork and reshape

---

## 1. Overview

A local-first, browser-native equity research dashboard built in Python + Streamlit + Plotly. Clones from GitHub, runs with one command, displays a sophisticated multi-tab dashboard for a pre-configured watchlist (initially: a starter list — NVDA, AAPL, MSFT, GOOGL, TSLA — the user edits). Operated via natural-language prompts to the user's AI coding agent (Claude Code in VS Code). Designed to work with **zero API keys** out of the box (via yfinance) and gracefully upgrade when the user adds Finnhub, Alpha Vantage, or Claude API keys.

This is also the **canonical template** for future users who want a similar domain-specific tool. Build it so it's forkable and easily reshaped for other verticals (e.g. sports analytics, real estate).

---

## 2. Target user

A **non-technical user** who is AI-fluent but not code-fluent — comfortable talking to an AI assistant, not comfortable with a terminal. Equities is a personal interest, not a profession; this tool is a relaxed research companion, not a professional trading system.

**Critical constraints on their experience:**

- **They do not type commands.** They talk to their Claude Code agent in natural language.
- **They do not know what `pip install`, `git clone`, `cd`, or `.env` mean.** Their agent handles all of that.
- **Assume macOS.** All setup and error-handling guidance is written Mac-first (`python3`, `brew`, Homebrew-preferred install paths, TextEdit rich-text trap, Finder extension-hiding). Windows guidance is kept as a secondary aside where relevant.
- **Starter watchlist:** NVDA, AAPL, MSFT, GOOGL, TSLA. The user swaps in the tickers they care about via `config/tickers.yaml`.
- **A Claude API key is optional.** When present, it unlocks the AI briefing feature; the tool is fully functional without it.
- **Crypto-free by default.** Do not include crypto examples or demo tickers unless the user explicitly asks.

### Learning style

- Learns by seeing **concrete results on screen** (a chart updates, a number appears, a page changes) — not abstract frameworks
- Demos that produce visible change ("a price appears") land; abstract framework-first lessons do not
- Already trusts AI to do real work — comfortable directing an agent to build and reshape the tool

---

## 3. Core UX principle — agent-operated, not human-operated

**The user never types a command.** Every action — cloning, installing, running, extending — happens because they tell their Claude Code agent in plain English what they want.

This reshapes the whole project:

- The README is written for the user to paste to their agent, not for them to execute
- `PROMPTS.md` is a cookbook of example sentences they can say
- `CLAUDE.md` at the project root teaches whatever agent runs in their project how the code is organized, how to extend it, and what conventions to follow
- Error messages are written so their agent can read them and diagnose
...
1. **Zero-key start.** The user can clone the repo and, with zero API keys configured, have a working dashboard displaying live data for their watchlist within 5 minutes of saying "set this up for me" to their agent.
...
4. **Extensibility.** The user can say "add TSLA to my watchlist" or "add a sector breakdown view" to their agent and the agent can do it by following documented patterns in `CLAUDE.md` — without rewriting core code.
...
3. **Comparison** — all watchlist tickers side-by-side: radar chart of normalized ratios (P/E, P/B, ROE, dividend yield, market cap), bar chart of YTD returns, table of key metrics
4. **AI Briefing** — "Generate weekly briefing" button; if `ANTHROPIC_API_KEY` is set, calls Claude API with structured context about the watchlist stocks; output is a markdown briefing covering themes, risks, opportunities, earnings watch list; cached to `briefings/YYYY-MM-DD.md`; falls back to a "Set up AI briefings" message if no key
...
Input to Claude: structured JSON payload with all watchlist tickers' data from the dashboard (prices, key ratios, recent news headlines, upcoming earnings).
...
- **Earnings calendar** (upcoming earnings they should watch)
- **Questions to sit with** (3-4 open questions for their research journal)
...
The user's Anthropic account must be protected from accidental overspend. Every one of the following must be implemented — not optional:
...
3. **Confirmation dialog every call.** When they click, a Streamlit `st.dialog` or equivalent shows: *"This will use approximately $0.05 of your Anthropic API budget. You've used $X.XX of $5.00 this month. Continue?"* with Confirm / Cancel.
...
5. **Hard monthly cap.** When `ANTHROPIC_MONTHLY_BUDGET_USD` is reached, the button is disabled with: *"Monthly Anthropic budget reached. The button will re-enable on [1st of next month] or when you raise the cap in your .env file."* No retries, no override button. They have to change `.env` manually (their agent can help).
...
Acceptance test: a malicious or buggy caller trying to call the Claude endpoint 100 times in a loop must be structurally impossible. Budget overrun requires the user to manually edit `.env` — not something they can do by accident in the UI.
...
- **Python 3.11+** (the user's Mac may ship with 3.9 Apple-supplied — install `python@3.11` via Homebrew or python.org installer)
...
- Covers the watchlist reliably (US stocks, ETFs, ADRs)
...
3. **In the Streamlit app itself** — if a key is missing, the relevant tab/feature shows a clear inline instruction telling the user what to say to their agent
...
├── README.md                     # For the user — what to say to their agent
...
├── config/
│   ├── tickers.yaml              # Watchlist (NVDA seeded)
...
|---|---|---|
| README.md | Setup instructions phrased as prompts | User (via copy-paste to their agent) |
...
A curated list of natural-language prompts the user can say to their Claude Code agent. Grouped by task. Each prompt is a complete sentence they can paste verbatim. The file teaches by example.
...
- "Remove a ticker from my watchlist"
...
This file sits at the project root. When the user opens Claude Code in their cloned project, Claude Code reads this on session start and understands:

1. **What this project is** — 1 paragraph
2. **Who the user is** — a non-technical user, operates via prompts
...
8. **When to ask the user** — bullet list of decisions the agent should run past the user before acting (e.g. "before changing the theme or adding a paid API key")
...
## 10. Setup flow (what happens when their agent gets the GitHub link)

1. The user says to their Claude Code: *"Clone this repo and get it running: [url]"*
...
6. Agent runs `streamlit run app.py` — dashboard opens in their browser
7. Agent tells the user: *"Your dashboard is running at http://localhost:8501. Your watchlist is loaded. Try saying things like 'add TSLA' or 'show me my comparison view'."*
...
## 11. Extensibility framework — how they iterate

The PRD's promise is: once the base tool is shipped, the user can say natural things to their agent and the agent extends the tool by following the documented patterns.
...
- README + HELP.md tell the user's agent: never ask the user to edit `.env` in a GUI text editor (TextEdit on Mac, Notepad on Windows). Always edit via the agent's own file tools, or `nano`/`code` from Terminal.
...
- Seed `config/tickers.yaml` with a starter watchlist (NVDA + 4 mega-caps)
...
- At end of Phase 1, the user can clone, run, and see their watchlist with live data
...
- `views/comparison.py` — all watchlist tickers side by side with radar chart + YTD bars
...
- Fresh-clone smoke test on a separate machine (target: macOS; Windows second-pass), run through setup flow as a non-technical user
...
- Correlation matrix between their holdings
- Export briefing to PDF
- Multi-portfolio support (e.g. "my stocks" vs "watchlist")
- Mobile-responsive layout tweaks

Mention these in the README so the user knows the tool has room to grow — they should feel empowered to add them by asking their agent.
...
- **Not hosted.** Runs on their laptop, not deployed anywhere. They don't need to worry about servers, domains, or hosting costs.
...
- **Not an educational tool.** It's a product. Learning happens when the user asks their agent to modify it, not through tutorials in the UI.
...
1. **Repo visibility — PUBLIC** at `github.com/seanmccloskey10-cell/equity-research-desk`.
   - **Rationale:** A private repo would force the user through GitHub auth (PAT or SSH keys) to clone — friction a non-technical user can't handle without being walked through it, and every future user hits the same wall. Public removes friction entirely: their agent runs `git clone <url>` with no auth. Nothing sensitive is in the repo itself — `.env` is gitignored, there are no secrets in source. The code being public is a feature, not a risk: future users benefit, the author gets attribution, and the template pattern (§16.7) only works cleanly with public.
2. **README audience — the agent.** The README is written for the user's Claude Code agent to parse, not for the user to read. Be as technical as needed (exact commands, paths, versions, code examples). Clarity for the agent > readability for humans. The user will only ever type something like *"Clone this and set it up for me: <url>"* — their agent reads the README and executes.
