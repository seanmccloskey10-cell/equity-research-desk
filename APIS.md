...
Full directory of financial data APIs the user can evaluate. Organized by what problem each one solves. **All crypto APIs deliberately excluded** by default.
...
These ship with the tool. Everything below is optional and can be added.
...
- **Gives the user:** deep fundamentals, income statements, balance sheets, cash flows, financial ratios, analyst estimates, insider trading data
...
- **Gives the user:** interest rates, inflation, GDP, employment, housing, any macro indicator
- **Free tier:** unlimited (just need a free API key)
- **Paid:** N/A — always free
- **Why it matters:** Helps the user understand *why* stocks move — Fed decisions, inflation prints, etc.
- **URL:** https://fred.stlouisfed.org/docs/api/fred/
- **Python:** `pip install fredapi`

### 3. Quartr — **earnings call transcripts**
- **Gives the user:** full earnings call transcripts, investor presentations
- **Free tier:** limited
- **Paid:** starts ~$20/mo
- **Why it matters:** Earnings transcripts are where real research lives. Much richer than headlines.
- **URL:** https://quartr.com/
- **Note:** This is also available as a **Cowork connector** — if the user has Claude Pro/Max, they may be able to use it from Cowork directly without adding it to this tool.
...
| Twelve Data | 800/day, 8/min | $29/mo | Covers forex too if the user ever wants that. |
...
| EIA (US Energy Info) | Unlimited | N/A | Energy prices, production. Useful for energy names. |
...
| API | Free tier | Paid starts | Notes |
|---|---|---|---|
| TradingView | N/A (charts embed only) | $14.95+/mo for API | Excellent charting if the user ever wants embedded charts. |
| Koyfin | Web app, no API | $35/mo | **Not an API — web dashboard.** If the user wants the "no-code" path instead of building, this is the consumer-grade answer. Worth knowing about. |
...
When the user considers adding one, their agent should walk them through these questions:

1. **What specific data does this give me that I don't already have?**
2. **Is the free tier enough for my usage pattern?** (The user runs briefings maybe daily/weekly — hundreds of calls/day would be wildly over-provisioned.)
3. **What's the paid tier trigger?** (e.g. "I'd only pay for this if I wanted real-time data for day-trading" — they don't, so free tier is fine.)
...
| The question | Best free pick | Best paid pick (under $25/mo) |
|---|---|---|
